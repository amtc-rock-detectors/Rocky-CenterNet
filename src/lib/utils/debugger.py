...
class Debugger(object):
    def __init__(self, ipynb=False, theme='black', 
        ...
        if dataset == 'coco_hp':
            ...
            self.W = 1242
            self.H = 375
        elif 'rock_coco' in dataset:
            self.names = rock_class_names
    ...
    def add_coco_ellipse(self, bbox, cat, conf=1, show_txt=False, img_id='default'):
        #bbox = np.array(bbox, dtype=np.int32)
        ...
        b = bbox[3]
        ang = bbox[4]
        if ang == 0.25 or ang == 0.75:
            ang += 0.05 * np.random.rand()
        ang *= np.pi

        if np.any(np.isnan([x_c, y_c, a, b, ang, conf])) or a <= 0 or b <= 0:
           return
        cv2.ellipse(
            self.imgs[img_id], (int(x_c), int(y_c)), (int(a / 2), int(b / 2)),
            ang * 180 / np.pi, 0, 360, c, 2)
        if show_txt:
            cv2.rectangle(self.imgs[img_id],
                          (bbox[0], bbox[1] - cat_size[1] - 2),
                          (bbox[0] + cat_size[0], bbox[1] - 2), c, -1)
    ...
    def add_coco_hp(self, points, img_id='default'): 
        ...
        for j in randge(self.num_joints):
    ...
    def save_img(self, imgId='default', path='./cache/debug/image.png'):
