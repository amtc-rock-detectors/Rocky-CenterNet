...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('task', default='rockdet',
                                                         help='ctdet | ddd | multi_pose | exdet | rockdet')
        self.parser.add_argument('--dataset', default='rock_coco',
                                                         help='coco | kitti | coco_hp | pascal | rock_coco')
        ...
        self.parser.add_argument('--keep_res', action='store_true',
                                                         help='keep the original resolution'
                                                                    ' during validation.')
        self.parser.add_argument('--use_gt_poly', action='store_true',
                                                         help="Use original polygon as gt")
                                                         
        self.parser.add_argument('--use_gt_bbox', action='store_true',
                                                         help="Use original bbox as gt")
        ...
        # rockdet + ctdet rock_coco dataset
        self.parser.add_argument('--ang_anchors', type=int, default=5,
                                 help='Number of anchor levels, 0 | >= 2')
        self.parser.add_argument('--ang_weight', type=float, default=0.2,
                                 help='loss weight for ellipse angle.')
        self.parser.add_argument('--min_area', type=float, default=-np.inf,
                                 help='minimum ellipse area to be considered as detection')
        self.parser.add_argument('--max_area', type=float, default=np.inf,
                                 help='maximum ellipse area to be considered as detection')
        self.parser.add_argument('--small_min', type=float, default=0,
                                 help='Minimum area for small ellipse')
        self.parser.add_argument('--small_max', type=float, default=-1,
                                 help='Maximum area for small ellipse')
        self.parser.add_argument('--medium_min', type=float, default=-1,
                                 help='Minimum area for medium ellipse')
        self.parser.add_argument('--medium_max', type=float, default=-1,
                                 help='Minimum area for medium ellipse')
        self.parser.add_argument('--large_min', type=float, default=-1,
                                 help='Minimum area for medium ellipse')
        self.parser.add_argument('--large_max', type=float, default=1e10,
                                 help='Minimum area for medium ellipse')                
        # multi_pose
        ...
        self.parser.add_argument('--eval_oracle_ang', action='store_true',
                                                         help='use ground truth angle value.')
    ...
    def parse(self, args=''):
        ...
        if opt.dataset == "rock_coco":

            if opt.task == 'rockdet': 
                opt.dataset = "rock_coco_ellipse"
            elif opt.task == 'ctdet': 
                opt.dataset = "rock_coco_bbox"
            else: 
                raise ValueError(f"Invalid task / dataset combination, got {opt.task} / {opt.dataset}")
        
        if opt.task == 'rockdet' or opt.task == 'ctdet':
        
            cand_dict = {'small': [opt.small_min, opt.small_max], 
                         'medium': [opt.medium_min, opt.medium_max,], 
                         'large': [opt.large_min, opt.large_max],
                         'all': [opt.small_min, opt.large_max]}
                
            if np.any([np.any(np.array(cand_dict[key]) < 0) for key in cand_dict.keys()]):
                cand_dict = None           
            
            opt.areaRng = cand_dict
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        if opt.task == 'exdet':
            ...
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
        elif opt.task == 'rockdet':
            # assert opt.dataset == "rock_coco"
            assert opt.ang_anchors == 0 or opt.ang_anchors >= 2
            ang_levels = opt.ang_anchors + 1
            opt.heads = {'hm': opt.num_classes,
                         'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes,
                         'ang': ang_levels if not opt.cat_spec_wh else \
                         ang_levels * opt.num_classes}
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
    ...
    def init(self, args=''):
        default_dataset_info = {
            'ctdet': {'default_resolution': [512, 512], 'num_classes': 80, 
                                ...
                                'dataset': 'coco'},
            'rockdet': {'default_resolution': [512, 512], 'num_classes': 1,
                        'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                        'dataset': 'rock_coco_ellipse'},
